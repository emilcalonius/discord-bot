...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addIntegerOption((option) =>
      option
        .setName('limit')
        .setDescription(
          'How many messages back you want to check rounded to the nearest 100'
        )
        .setMinValue(1)
        .setMaxValue(10000)
        .setRequired(true)
  ...
  async execute(interaction) {
    ...
    const limit = Math.ceil(
      (interaction.options.getInteger('limit') / 100) * 100
    )
    let spams = []
    ...
    await interaction.reply(
      `Searching for instances of 10 or more gifs in a row from the last ${limit} message${
        limit > 1 ? 's' : ''
      }... 🔍`
    )
    ...
    for (let i = 0; i <= limit; i += 100) {
      ...
      messages.reverse().forEach((message) => {
        if (
          message.content.includes('https://giphy.com/') ||
      ...
      for (let i = 0; i < spams.length; i += len) {
        ...
        await interaction.followUp(
          `${temp
            .map(
              (spam) =>
                `• [${spam.createdAt.toLocaleString('fi-FI', {
                  day: 'numeric',
                  month: 'numeric',
                  year: 'numeric',
                  hour: 'numeric',
